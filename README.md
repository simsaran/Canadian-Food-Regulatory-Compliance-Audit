# Canadian Food Regulatory Compliance Audit

Regulatory compliance assessment of 10 PepsiCo Canada consumer food products against Canadian Food & Drug Regulations (FDR), CFIA labelling requirements, and the 2026 Front-of-Package (FOP) nutrition symbol rules.

> **Disclaimer:** This is an independent educational audit conducted for skills demonstration purposes. Findings are based on publicly available product labels purchased in Ontario, Canada (April 2026) and may not reflect the most current packaging in market.

## Background

On January 1, 2026, Health Canada's amendments to the Food and Drug Regulations took full effect, requiring mandatory Front-of-Package (FOP) nutrition symbols on prepackaged foods high in saturated fat, sugars, or sodium (≥15% Daily Value). This project audits 10 products for compliance with this and other Canadian labelling requirements.

## Products Audited

| #  | Product                                      | Category |
| -- | -------------------------------------------- | -------- |
| 1  | Lay's Classic Potato Chips                   | Snack    |
| 2  | Doritos Nacho Cheese Tortilla Chips          | Snack    |
| 3  | Tostitos Scoops Tortilla Chips               | Snack    |
| 4  | Quaker Instant Oatmeal (Maple & Brown Sugar) | Cereal   |
| 5  | Quaker Chewy Granola Bar (Chocolate Chip)    | Snack    |
| 6  | Tropicana Pure Premium Orange Juice          | Beverage |
| 7  | Gatorade Lemon-Lime                          | Beverage |
| 8  | Ruffles All Dressed Potato Chips             | Snack    |
| 9  | SunChips Harvest Cheddar                     | Snack    |
| 10 | Miss Vickie's Sea Salt & Malt Vinegar        | Snack    |

## Assessment Categories

Each product was assessed across 6 regulatory categories:

1. **Ingredient Statement** — FDR B.01.008.2 compliance
2. **Nutrition Facts Table** — FDR B.01.350 compliance
3. **FOP Nutrition Symbol** — ≥15% DV threshold
4. **Allergen Declaration** — FDR B.01.010.1 compliance
5. **Claims Substantiation** — FDR B.01.503
6. **Bilingual Compliance**

## Key Findings

### FOP Symbol Requirement

* **5 out of 10 products** require the mandatory FOP nutrition symbol
* **Sodium** is the primary trigger (Doritos 16% DV, Ruffles 18% DV, Miss Vickie's 18% DV)
* **Sugars** trigger FOP for beverages (Tropicana 22% DV, Gatorade 34% DV)
* Tropicana 100% orange juice triggers FOP for **naturally occurring sugars**, as the regulation does not distinguish between natural and added sugars
* FOP compliance is **subject to packaging print date**, as CFIA permits pre-2026 inventory to remain in market

### Allergen Compliance

* All 10 products are compliant with FDR B.01.010.1
* Miss Vickie's contains **lactose (milk derivative)** and correctly declares **"Contains Milk and Barley Ingredients"**, demonstrating proper allergen cross-referencing
* Quaker Chewy includes a comprehensive precautionary allergen statement covering additional risks

### Claims Substantiation

* All claims were validated against FDR thresholds
* SunChips meets the **minimum threshold (4g fibre)** for “High Source of Fibre”
* Quaker Oatmeal health claim aligns with beta-glucan requirements

### Compliance Summary

| Category              | Compliant | Assessed | Rate     |
| --------------------- | --------- | -------- | -------- |
| Ingredient Statement  | 10        | 10       | 100%     |
| Nutrition Facts Table | 10        | 10       | 100%     |
| Allergen Declaration  | 10        | 10       | 100%     |
| FOP Symbol            | 5         | 5        | Pending* |
| Claims Substantiation | 10        | 10       | 100%     |
| Bilingual Compliance  | 10        | 10       | 100%     |

*FOP compliance depends on packaging production date and requires physical verification.

### Key Insight

All identified gaps relate to **transitional implementation of FOP regulations**, not intrinsic labeling non-compliance.

## Methodology

Standardized assessment procedure documented in [SOP-REG-001](docs/SOP-REG-001.pdf). CAPA reports developed using 5 Whys root cause analysis.

## Deliverables

| Document          | Description                  |
| ----------------- | ---------------------------- |
| Compliance Matrix | Full audit dataset           |
| Python Analysis   | Compliance visualizations    |
| SOP-REG-001       | Standardized procedure       |
| CAPA Reports      | Root cause analysis findings |
| Label Photos      | Evidence documentation       |

## Regulations Referenced

* Food and Drugs Act (FDA) & Food and Drug Regulations (FDR)
* Safe Food for Canadians Regulations (SFCR)
* CFIA Industry Labelling Tool
* Health Canada FOP Amendments (Canada Gazette Part II, 2022)

## Skills Demonstrated

Regulatory compliance analysis | FOP assessment | Allergen validation | Claims verification | Root cause analysis | CAPA documentation | SOP development | Data visualization (Python)

## Author

Simran Saran
B.Sc. Biochemistry, University of Waterloo
[s4saran@uwaterloo.ca](mailto:s4saran@uwaterloo.ca)
